...
class AiService:
    ...
    async def customer_reply(self, activity: TrainingActivity, messages: list[PracticeMessage]) -> str:
        ...
        try:
            payload = await self._chat(self._customer_messages(activity, messages), temperature=0.7)
...
{{
  "total_score": 0-100整数,
  "dimension_scores": [{{"dimension_id": 数字, "name": "维度名", "weight": 数字, "score": 0-100整数, "evidence": "证据", "suggestion": "建议"}}],
  "strengths": ["优点"],
  "issues": ["问题"],
...
""".strip()
    ...
    def _customer_messages(self, activity: TrainingActivity, messages: list[PracticeMessage]) -> list[dict[str, str]]:
        api_messages = [{"role": "system", "content": self._customer_prompt(activity)}]
        for message in messages[-12:]:
            api_messages.append({"role": "assistant" if message.role == "ai_customer" else "user", "content": message.content})
        return api_messages

    def _customer_prompt(self, activity: TrainingActivity) -> str:
        persona = activity.persona
        scripts = "\n".join([f"- [{item.item_type}] {item.title}: {item.content}" for item in activity.script_items])
...
每次回复 1-3 句话，像真实客户一样自然追问。若学员承诺审批通过、诱导包装资料或弱化费用风险，你可以继续追问细节，但不要替学员纠错。
""".strip()
